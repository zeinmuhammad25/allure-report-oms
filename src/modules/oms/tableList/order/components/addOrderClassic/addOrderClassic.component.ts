...
import AddOrderLocator from "./addOrderClassic.locator";
import AddOrderScenario from "./addOrderClassic.scenario";
...
export default class AddOrderComponent extends BaseOmsPage implements AddOrderScenario {
    ...
    shouldHave(): Element[] {
        return [
            Element.ofSelector(AddOrderLocator.fieldSearch),
            Element.ofSelector(AddOrderLocator.buttonRefresh),
            ...
            Element.ofSelector(AddOrderLocator.buttonApply),
            Element.ofSelector(AddOrderLocator.buttonCancel)
        ];
    }
    ...
    async applyMenuDetailPackage(): Promise<void> {
        ...
        await this.expectVisible(AddOrderLocator.buttonApply);
        await this.click(AddOrderLocator.buttonApply);
        await this.wait(1000);
    }

    async cancelMenuDetailPackage(): Promise<void> {
        await this.expectVisible(AddOrderLocator.buttonCancel);
        await this.click(AddOrderLocator.buttonCancel);
    }
