...
test.describe.serial("Ordering Dine In Split Bill", () => {
    ...
    test("[TC_0205251] Validate Logic when User can delete splitted Split Bill from the Main Bill",
        {tag: tags + "@negative"}, async ({tableList, bookOrder, order, splitBill}, testInfo) => {
            await safeTest(async ({tableList, bookOrder, order, splitBill}) => {
                ...
                await tableList.selectTableSplitBill("Main Bill");
                await order.splitBill();
                await splitBill.deleteBill("CHILD 1");
                await splitBill.closeSplitBill();
                await order.saveOrder();
            }, {tableList, bookOrder, order, splitBill}, testInfo);
        });

    test("[TC_0205252] Validate Logic when User can rename main Split Bill with special charaters only",
        {tag: tags + "@negative"}, async ({tableList, bookOrder, order, splitBill},testInfo) => {
            await safeTest(async ({tableList, bookOrder, order, splitBill}) => {
                await tableList.selectRoom(Table.acRoom.name);
                await tableList.selectTable(Table.acRoom.ac1.name);
    ...
    test("[TC_0205254] Validate Logic when User can Split Bill > 1 ordered menu",
        {tag: tags + "@negative"}, async ({tableList, bookOrder, order, splitBill}, testInfo) => {
            await safeTest(async ({tableList, bookOrder, order, splitBill}) => {
                await tableList.selectRoom(Table.acRoom.name);
                await tableList.selectTable(Table.acRoom.ac1.name);
    ...
    test("[TC_0205256] Validate Logic when User can Split Bill empty Child (Splitted) Bill",
        {tag: tags + "@negative"}, async ({tableList, bookOrder, order, splitBill}, testInfo) => {
            await safeTest(async ({tableList, bookOrder, order, splitBill}) => {
                await tableList.selectRoom(Table.acRoom.name);
                await tableList.selectTable(Table.acRoom.ac1.name);
    ...
    test("[TC_0205261] Validate Logic when User cannot Split Bill the Child Merge Table",
        {tag: tags + "@negative"}, async ({tableList, bookOrder, order, mergeTable, splitBill}, testInfo) => {
            await safeTest(async ({tableList, bookOrder, order, mergeTable, splitBill}) => {
                await tableList.selectRoom(Table.acRoom.name);
                await tableList.selectTable(Table.acRoom.ac1.name);
                await makeOrder("AT INCLUSIVE", bookOrder);
                await selectMultipleMenuBiasa(order, 10, 10, 10);
                ...
                await order.splitBill();
                await splitBill.addBill("CHILD 1");
                await splitBill.moveMenu("CHILD 1", MenuList.menus.atMenuBiasaGoreng.name, "4");
                await splitBill.closeSplitBill();
                await order.saveOrder();
                await tableList.selectRoom(Table.acRoom.name);
                await tableList.selectTable(Table.acRoom.ac1.name);
