    ...
    test("[TC_0205186] Validate Logic when User can Move Item to the empty table with the same Sales Mode",
        {tag: tags + "@positive"}, async ({bookOrder, order, tableList, moveItem, paymentV2}, testInfo) => {
            await safeTest(async ({bookOrder, order, tableList, moveItem, paymentV2}) => {
                ...
                await order.moveItem();
                await moveItem.moveItemToSectionDineIn(Table.acRoom.name, Table.acRoom.ac2.name);
                await moveItem.movePartialItemMenu(MenuList.menus.atMenuBiasaBakar.name, 3);
                await moveItem.actionApplyMoveItem();
    ...
    test("[TC_0205189] Validate Logic when User can Move Item to the empty table with a Menu selected",
        {tag: tags + "@positive"}, async ({bookOrder, order, tableList, moveItem, paymentV2}, testInfo) => {
            await safeTest(async ({bookOrder, order, tableList, moveItem, paymentV2}) => {
                ...
                await order.moveItem();
                await moveItem.moveItemToSectionDineIn(Table.acRoom.name, Table.acRoom.ac2.name);
                await moveItem.movePartialItemMenu(MenuList.menus.atMenuBiasaBakar.name, 5);
                await moveItem.actionApplyMoveItem();
                await moveItem.verifyCurrentQty(MenuList.menus.atMenuBiasaBakar.name, 6);
                await order.printNowPrintingSetting();
                await order.gotoPayment();
    ...
    test("[TC_0205201] Validate Logic when User can Move Item to filled table from Parent Merge Table",
        {tag: tags + "@positive"}, async ({bookOrder, order, tableList, moveItem, mergeTable, paymentV2}, testInfo) => {
            await safeTest(async ({bookOrder, order, tableList, moveItem, mergeTable, paymentV2}) => {
                ...
                await moveItem.movePartialItemMenu(MenuList.menus.atMenuBiasaRebus.name, 2);
                await moveItem.actionApplyMoveItem();
                await moveItem.verifyCurrentQty(MenuList.menus.atMenuBiasaRebus.name, 6);
                await order.saveOrder();
                await tableList.selectRoom(Table.acRoom.name);
                await tableList.selectTable(Table.acRoom.ac3.name);
                await order.printNowPrintingSetting();
