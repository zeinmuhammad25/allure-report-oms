...
test.describe.serial("Cancel And Void Report Test", () => {
    ...
    test("[TC_0402016] Verify User can filter and show Report Cancel And Void when Cancel Table",
        {tag: tag}, async ({page}) => {
            const cancelAndVoidPage = new CancelAndVoidPage(page);
            await cancelAndVoidPage.validateCancelAndVoidWhenCancelTable();
        });

    test("[TC_0402017] Verify User can filter and show Report Cancel And Void when Void Transaction",
        {tag: tag}, async ({page}) => {
            const cancelAndVoidPage = new CancelAndVoidPage(page);
            await cancelAndVoidPage.validateCancelAndVoidWhenVoidTransaction();
        });

    test("[TC_0402018] Verify User can filter, show, and download Report Cancel And Void",
        {tag: tag}, async ({page}) => {
            const cancelAndVoidPage = new CancelAndVoidPage(page);
