...
test.describe.serial("Sales Menu Report Test", () => {
    ...
    test("[TC_0402019] Verify User can filter and show Data Sales Menu on Sales Menu Report",
        {tag: tag}, async ({page}) => {
            const salesMenuPage = new SalesMenuPage(page);
            await salesMenuPage.validateFilterAndShowDataSalesMenuOnSalesMenu();
        });

    test("[TC_0402020] Verify User can filter, show, and download Sales Menu Report",
        {tag: tag}, async ({page}) => {
            const salesMenuPage = new SalesMenuPage(page);
