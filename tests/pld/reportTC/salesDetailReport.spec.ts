...
test.describe.serial("Sales Detail Report Test", () => {
    ...
    test("Verify User can filter and view data Sales Detail", {tag: tag}, async ({page}) => {
        const salesDetailPage = new SalesDetailPage(page)
        await salesDetailPage.validateFilterAndShowData()
    })

    test("Verify User can filter and view data Sales Detail with Transaction Number", {tag: tag}, async ({page}) => {
        const salesDetailPage = new SalesDetailPage(page)
        await salesDetailPage.validateShowDataWithTransactionNumber()
    })

    test("Verify User can filter, view, and download data", {tag: tag}, async ({page}) => {
        const salesDetailPage = new SalesDetailPage(page)
