...
test.describe.serial("Dashboard Composition Test", () => {
    ...
    test("[TC_0401012] Verify User can filter and show sales by Sales Mode Data of Dashboard Composition",
        {tag: tag}, async ({page}) => {
            const dashboardPage = new DashboardPage(page);
            await dashboardPage.validateSalesBySalesModeDataOnDashboardComposition();
        });

    test("[TC_0401013] Verify User can filter and show sales by Payment Method Data of Dashboard Composition",
        {tag: tag}, async ({page}) => {
            const dashboardPage = new DashboardPage(page);
            await dashboardPage.validateSalesByPaymentMethodDataOnDashboardComposition();
        });

    test("[TC_0401014] Verify User can filter and show sales by Category Data of Dashboard Composition",
        {tag: tag}, async ({page}) => {
            const dashboardPage = new DashboardPage(page);
            await dashboardPage.validateSalesByCategoryDataOnDashboardComposition();
        });

    test("[TC_0401015] Verify User can filter and show Top Menu Data of Dashboard Composition",
        {tag: tag}, async ({page}) => {
            const dashboardPage = new DashboardPage(page);
            await dashboardPage.validateTopMenuDataOnDashboardComposition();
        });

    test("[TC_0401016] Verify User can filter and show Top Branch Data of Dashboard Composition",
        {tag: tag}, async ({page}) => {
            const dashboardPage = new DashboardPage(page);
